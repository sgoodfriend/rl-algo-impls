...
They tried training with self-play, where the policy plays against itself.
They found self-play didn't improve the win rate, neither when only using self-play nor
when training with half self-play and half scripted bots. We found a bug where resources
(which should be unowned) were being counted as owned by the opponent if the agent was
the second player, which likely contributed to their finding no improvement.
...
value estimates. If future competitions change the Open maps, fine-tuning and transfer
learning will significantly help \agentName\ and \bcPPOAgent. We didn't train
