...
\begin{table}[ht]
    ...
    (4)BloodBath.scmB & \textbf{100} & \textbf{88} & 0 & 5 & \textbf{48} \\
    \hline
...
inference 2-3 times faster in the LUX competition. This performance gain would likely
make running DoubleCone or the larger squnet models on all maps except the largest
feasible for the competition. An overtime budget for an entire match instead of a 20 ms
per turn buffer will help agents deal with environment instabilities. For example,
\bcPPOAgent\ timedout in 11\% of games on \texttt{(4)BloodBath.scmB}, despite averaging
55.1 ms/turn and going over 100 ms in only 0.016\% of turns (averaging less than 1 over
100 ms turn per game). An overage budget of even 1 second per game would likely prevent
most timeouts.

\subsection{Training on larger maps}
None of the agents managed to reliably defeat the prior two competition winners on 
\texttt{(4)BloodBath.scmB}. The Mayari behavior cloning \bcAgent\ did best beating
CoacAI and Mayari only 20\% and 40\%, respectively. \texttt{(4)BloodBath.scmB} is a
likely particularly challenging map for reinforcement learning because game lengths are
significantly longer than on smaller maps (\bcPPOAgent\ averaged 3,500 steps per game on
...
RL is struggling to learn.

Behavior cloning was meant to bootstrap training by starting with a model that is able
to generate non-zero win-loss rewards and reasonable observation-action pairs. However,
while the training policy started by winning 40-50\% of games (1/6 of training
environments are against scripted bots and 1/4 are against older model weights of itself), this dropped to 20\%
midway through training. The training policy eventually recovers to winning 40\% of
games; however, the training policy had a worse evaluation win rate than the initial 
behavior cloned policy. This training curve differs from the smaller map training where the training
policy quickly reaches winning 60\% of training games and maintained before improving
upon the evaluation win rate.

PPO (and RL algorithms in general) have many hyperparameters that need to be tuned
before a model is able to learn. Hyperparameter search is ongoing. At the same time,
other algorithms might perform better from a behavior cloned start. For example,
...
transition from behavior cloning to RL.

\subsection{Behavior cloning, transfer learning, and academic competitions}
Training \agentName\ from random weights to multiple policies for different map sizes and fine-tuned
on certain maps took 70 GPU-days. Behavior cloning and PPO fine-tuning \bcPPOAgent\
took 73 GPU-days (\bcAgent\ took 23 GPU-days). These are significant amounts of compute
for a mostly academic competition like microRTS.

There are several ways to make training deep neural networks more feasible in a
competition and educational setting:
\begin{inparaenum}[(1)]
    \item focus on smaller maps,
    \item fine-tuning existing models that have been trained (possibly through BC, RL,
...
training on the larget map to little benefit. \citet{DBLP:journals/corr/abs-2105-13807}
trained an agent for a player 1 on a single 16x16 map in 60 hours. \bcPPOAgent\ trained
on the 5 competition maps up to size 16x16 in about 7.5 days.
...
policy already takes reasonable tactical actions and the critic can already make
reasonable value estimates.  While fine-tuning and transfer learning \agentName\ on the same maps
might not be took productive, both can be useful if the competition Open maps change and
could be useful for \bcPPOAgent\, which didn't do any single map training. Other agents,
such as the new baseline \texttt{2L}, could be behavior cloned and fine-tuned to
generate DRL agents with new behaviors.
...
to create competitive DRL agents with desired behaviors without needing to handcraft a reward
function.
...
\begin{table}[H]
    \centering
    \begin{threeparttable}
    \caption{Shaped fine-tuning schedule starting}
